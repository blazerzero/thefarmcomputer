...
export async function scrapeFruitTrees(): Promise<Omit<FruitTreeRow, "id" | "last_updated">[]> {
  ...
  for (const el of elements) {
    ...
    if (tag === "H3") {
      ...
      const treeLink = links.find((l) => !l.getAttribute("href")?.startsWith("/File:"));
    ...
    if (tableIndex === 1) {
      ...
      const headers = headerCells.map((th) => th.text.replace(/\s+/g, " ").trim().toLowerCase());

      const idxPierre = headers.findIndex((h) => h.includes("pierre"));
      const idxFruit = headers.findIndex((h) => h === "fruit");
      const idxSellPrice = headers.findIndex((h) => h.includes("sell price") || h.includes("fruit sell"));
      ...
      const fruitCell = idxFruit >= 0 && idxFruit < dataCells.length ? dataCells[idxFruit] : null;
      const fruitName = (fruitCell?.querySelector("a")?.text || fruitCell?.text || "")
        .replace(/\s+/g, " ")
        .trim() || null;
