...
function detectQuality(
	...
	if (
		combined.includes("iridium quality") ||
		combined.includes("quality_4") ||
		combined.includes("iridium star")
	)
		return "Gold"; // treat iridium as gold for display
	if (
		combined.includes("gold quality") ||
		combined.includes("quality_3") ||
		combined.includes("gold star")
	)
		return "Gold";
	if (
		combined.includes("silver quality") ||
		combined.includes("quality_2") ||
		combined.includes("silver star")
	)
		return "Silver";
	return undefined;
}

export async function scrapeBundles(): Promise<
	Omit<BundleRow, "id" | "last_updated">[]
> {
	...
	for (const el of elements) {
		...
		const nameLink = headerTh
			.querySelectorAll("a")
			.find((a) => !a.getAttribute("href")?.startsWith("/File:"));
		const bundleName = (nameLink?.text ?? headerTh.text)
			.replace(/\s+/g, " ")
			.trim();
		...
		for (let i = 1; i < rows.length; i++) {
			...
			for (const td of tds) {
				...
				const itemLink = td.querySelectorAll("a").find((a) => {
					const href = a.getAttribute("href");
					return (
						href && !href.startsWith("/File:") && !href.startsWith("/index.php")
					);
				});
