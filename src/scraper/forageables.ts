...
export async function scrapeForageables(): Promise<
	Omit<ForageableRow, "id" | "last_updated">[]
> {
	...
	if (!forageH2) {
		console.error(
			"Could not find 'Foraged Items' H2 on the Foraging wiki page",
		);
		return [];
	}
	...
	for (const el of elements) {
		...
		const idxSell = headers.findIndex(
		...
		for (let r = 1; r < rows.length; r++) {
			...
			const cellRichText = (idx: number): string =>
				idx >= 0 && idx < cells.length
					? String(mdEngine.processSync(cells[idx]!.innerHTML))
							.replaceAll(/\*\s*/gm, "")
							.trim()
					: "";
			...
			const imgSrc =
				imageCell?.querySelector("img")?.getAttribute("src") ?? null;
