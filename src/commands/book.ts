...
export function handleBook(
	...
	if (!book) {
		return notFoundResponse(
			`No book named **${name}** found. Check the spelling (e.g. \`Price Catalogue\`, \`Animal Catalogue\`).`,
		);
	}
	...
	return embedResponse({
		...
		fields,
		footer: {
			text: `Data from Stardew Valley Wiki • Last updated ${formatDate(book.last_updated)}`,
		},
	});
}
