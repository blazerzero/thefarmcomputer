...
export function handleIngredient(
	...
	const recipes: string[] = items.reduce((acc, curr) => {
		const idx = curr.ingredients.findIndex(
			(i) => i.name.toLowerCase() === name.toLowerCase(),
		);
		if (idx >= 0)
			acc.push(`${curr.name} (${curr.ingredients[idx]!.quantity} needed)`);
		return acc;
	}, [] as string[]);
	...
	return embedResponse({
		...
		footer: {
			text: `Data from Stardew Valley Wiki • Last updated ${formatDate(lastUpdated)}`,
		},
	});
}
