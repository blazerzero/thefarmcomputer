...
export function handleCraft(
  ...
  if (item.recipe_source !== null) {
    fields.push({ name: "Recipe Source", value: item.recipe_source, inline: false });
  }
  ...
  return embedResponse({
    ...
    fields,
    footer: { text: `Data from Stardew Valley Wiki • Last updated ${formatDate(item.last_updated)}` },
  });
}
