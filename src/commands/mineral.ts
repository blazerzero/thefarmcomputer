...
export function handleMineral(
	...
	if (!mineral) {
		return notFoundResponse(
			`No mineral named **${name}** found. Check the spelling (e.g. \`Quartz\`, \`Emerald\`, \`Frozen Geode\`).`,
		);
	}
	...
	if (mineral.sell_price_gemologist !== null) {
		fields.push({
			name: "Gemologist Price",
			value: `${mineral.sell_price_gemologist}g`,
			inline: true,
		});
	}
	...
	return embedResponse({
		...
		fields,
		footer: {
			text: `Data from Stardew Valley Wiki • Last updated ${formatDate(mineral.last_updated)}`,
		},
	});
}
